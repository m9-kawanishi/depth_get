...
class Application:
    def __init__(self):
        ...
        self.ex, self.ey = -1, -1
        self.dc = DepthCamera()
        self.mouse_pos = (-1, -1)  # マウス位置を初期化

    def mouse_callback(self, event, x, y, flags, param):
        if event == cv2.EVENT_MOUSEMOVE:
            self.mouse_pos = (x, y)  # マウスが動いた場合、その位置を更新
    ...
    def run(self):
        while True:
            ...
            if self.mouse_pos != (-1, -1):
                # マウスの位置情報をDepth Frameにテキストとして描画
                cv2.putText(depth_map, f'Pos: {self.mouse_pos}', (10, 30), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 2)

            cv2.imshow("Depth Frame", depth_map)
            cv2.imshow("Color Frame", color_frame)
            cv2.setMouseCallback("Color Frame", self.draw_rectangle)
            cv2.setMouseCallback("Depth Frame", self.mouse_callback)  # マウスイベントを設定
